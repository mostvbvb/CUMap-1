...
router.get('/atm', async (req, res) => {
    try {
        const connection = await getConnection();
        const [atms] = await connection.query('SELECT Latitude, Longitude FROM ATM A JOIN Position P ON A.Landmark_id = P.Landmark_id');
        // console.log(locations[0].Landmark_id);
...
router.get('/canteen', async (req, res) => {
    try {
        const connection = await getConnection();
        const [canteens] = await connection.query('SELECT Latitude, Longitude FROM Canteen C JOIN Position P ON C.Landmark_id = P.Landmark_id');
...
router.get('/museum', async (req, res) => {
    try {
        const connection = await getConnection();
        const [museums] = await connection.query('SELECT Latitude, Longitude FROM Museum M JOIN Position P ON M.Landmark_id = P.Landmark_id');
...
router.get('/coffeeshop', async (req, res) => {
    try {
        const connection = await getConnection();
        const [coffeeshops] = await connection.query('SELECT Latitude, Longitude FROM Coffee_shop C JOIN Position P ON C.Landmark_id = P.Landmark_id');
...
router.get('/copyprint', async (req, res) => {
    try {
        const connection = await getConnection();
        const [copyprints] = await connection.query('SELECT Latitude, Longitude FROM Copy_Print_shop C JOIN Position P ON C.Landmark_id = P.Landmark_id');
...
router.get('/vending_machine', async (req, res) => {
    try {
        const connection = await getConnection();
        const [vendms] = await connection.query('SELECT Latitude, Longitude FROM Vending_Machine V JOIN Position P ON V.Landmark_id = P.Landmark_id');
...
router.get('/library', async (req, res) => {
    try {
        const connection = await getConnection();
        const [libraries] = await connection.query('SELECT Latitude, Longitude FROM Library L JOIN Position P ON L.Landmark_id = P.Landmark_id');
